...
class TickerPrefilterTest {
  ...
  private final PrefilterProperties props =
      new PrefilterProperties(
          new BigDecimal("2000000"), new BigDecimal("0.001"), 14, Set.of("USDC", "WBTC", "WETH"));
  ...
  void allRulesPass_keeps() {
    ...
    List<Ticker> out =
        prefilter.filter(List.of(ticker("BTCUSDT", "78000", "793000000")), symbols, props, NOW);
  ...
  void volumeBoundary() {
    ...
    List<Ticker> out =
        prefilter.filter(
            List.of(ticker("EXACTUSDT", "1", "2000000"), ticker("UNDERUSDT", "1", "1999999")),
            symbols,
            props,
            NOW);
  ...
  void priceBoundary() {
    ...
    List<Ticker> out =
        prefilter.filter(
            List.of(
                ticker("EXACTUSDT", "0.001", "5000000"), ticker("DUSTUSDT", "0.0009", "5000000")),
            symbols,
            props,
            NOW);
  ...
  void missingMeta_dropped() {
    List<Ticker> out =
        prefilter.filter(List.of(ticker("GHOSTUSDT", "1", "5000000")), Map.of(), props, NOW);
  ...
  void notNormalStatus_dropped() {
    Map<String, SymbolMeta> symbols =
        Map.of("MAINTUSDT", new SymbolMeta("MAINTUSDT", "MAINT", 0L, "maintain"));

    List<Ticker> out =
        prefilter.filter(List.of(ticker("MAINTUSDT", "1", "5000000")), symbols, props, NOW);
  ...
  void freshListing_ageWindow() {
    Map<String, SymbolMeta> symbols =
        Map.of(
            "TENUSDT",
                new SymbolMeta(
                    "TENUSDT", "TEN", NOW.minus(Duration.ofDays(10)).toEpochMilli(), "normal"),
            "FIFTEENUSDT",
                new SymbolMeta(
                    "FIFTEENUSDT",
                    "FIFTEEN",
    ...
    List<Ticker> out =
        prefilter.filter(
            List.of(ticker("TENUSDT", "1", "5000000"), ticker("FIFTEENUSDT", "1", "5000000")),
            symbols,
            props,
            NOW);
  ...
  void deniedBaseCoin_dropped() {
    ...
    List<Ticker> out =
        prefilter.filter(List.of(ticker("USDCUSDT", "1", "5000000")), symbols, props, NOW);
  ...
  void substringIsNotDeny_hyundaiKept() {
    ...
    List<Ticker> out =
        prefilter.filter(List.of(ticker("HYUNDAIUSDT", "1", "5000000")), symbols, props, NOW);
