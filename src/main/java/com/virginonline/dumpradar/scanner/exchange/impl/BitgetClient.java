...
public class BitgetClient extends AbstractExchangeClient implements MarketDataClient {
  ...
  public BitgetClient(
      ExchangeProperties props, OkHttpClient http, ObjectMapper mapper, Clock clock) {
    super(Exchange.BITGET, props.exchanges().get(Exchange.BITGET).baseUrl(), http, mapper);
    this.clock = clock;
    this.symbolsCache = new TtlCache<>(props.cache().ttl(), clock, this::fetchSymbolsRaw);
  }
  ...
  public List<Candle> candles(String symbol, Timeframe timeframe, int limit) {
    List<Candle> raw =
        fetch(
            "/api/v2/mix/market/candles?symbol="
                + symbol
                + "&productType=USDT-FUTURES&granularity="
                + timeframe.code()
                + "&limit="
                + (limit + 1),
            Parser::parseCandles);
    if (!raw.isEmpty()
        && raw.getLast().openTime() + timeframe.duration().toMillis()
