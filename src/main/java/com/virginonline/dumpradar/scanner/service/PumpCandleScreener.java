...
public class PumpCandleScreener {
  ...
  private boolean bodyIsStrongEnough(Candle candle) {
    return candle
            ...
            .compareTo(props.minBodyPct())
        >= 0;
  }

  private boolean volumeSpikes(List<Candle> candles, Candle signal) {
    List<Candle> history =
        candles.subList(candles.size() - 1 - props.historyBars(), candles.size() - 1);
  ...
  private boolean closesNearHigh(Candle candle) {
    return candle
            .high()
            .subtract(candle.close())
            .compareTo(props.maxCloseFromHigh().multiply(candle.high().subtract(candle.low())))
        <= 0;
  }
}
